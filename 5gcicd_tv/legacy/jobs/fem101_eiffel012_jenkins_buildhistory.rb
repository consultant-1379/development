...
$jenkins_jobs_to_be_tracked_fem101_eiffel012 = []
...
def get_jenkins_data_history_fem101_eiffel012(name_jenkins, key)
...
URI_SUFFIX_HISTORY_FEM101_EIFFEL012 = get_jenkins_data_history_fem101_eiffel012('fem101-eiffel012-jenkins', 'uri_suffix')
JENKINS_URI_HISTORY_FEM101_EIFFEL012 = URI.parse(get_jenkins_data_history_fem101_eiffel012('fem101-eiffel012-jenkins', 'uri'))
USER_HISTORY_FEM101_EIFFEL012 = get_jenkins_data_history_fem101_eiffel012('fem101-eiffel012-jenkins', 'user')
PASS_HISTORY_FEM101_EIFFEL012 = get_jenkins_data_history_fem101_eiffel012('fem101-eiffel012-jenkins', 'pass')

JENKINS_AUTH_HISTORY_FEM101_EIFFEL012 = {
  'name' => USER_HISTORY_FEM101_EIFFEL012,
  'password' => PASS_HISTORY_FEM101_EIFFEL012
}

#function to get all job names from jenkins and save them in an array
def get_all_jobs_fem101_eiffel012
  http = Net::HTTP.new(JENKINS_URI_HISTORY_FEM101_EIFFEL012.host, JENKINS_URI_HISTORY_FEM101_EIFFEL012.port)
  request = Net::HTTP::Get.new("/#{URI_SUFFIX_HISTORY_FEM101_EIFFEL012}/api/json?tree=jobs[name]")
  if JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['name']
    request.basic_auth(JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['name'], JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['password'])
  end
  all_jobs = []
...
def trim_filename_fem101_eiffel012(filename)
...
def get_build_status_json_from_csv_file_fem101_eiffel012
  ...
  CSV.foreach(JOB_STATUS_HISTORY_FILENAME) do |job_status_history_row|
    ...
    job_history_entry["job_name"] = trim_filename_fem101_eiffel012(job_name)
...
def update_job_status_history_csv_file_fem101_eiffel012(latest_jenkins_job_status_map)
  ...
  if (File.file?(JOB_STATUS_HISTORY_FILENAME) == false)
    #build array
    $jenkins_jobs_to_be_tracked_fem101_eiffel012.each do |job_name|
      job_entry = Array.new
...
def search_for_job_in_full_job_list_json_fem101_eiffel012(json_summary, job_name, job_status_hash_table)
...
SCHEDULER.every '60s', :first_in => 0 do
  ...
  $jenkins_jobs_to_be_tracked_fem101_eiffel012 = get_all_jobs_fem101_eiffel012

  num_jobs = $jenkins_jobs_to_be_tracked_fem101_eiffel012.count()
  if num_jobs == 0
  ...
  http = Net::HTTP.new(JENKINS_URI_HISTORY_FEM101_EIFFEL012.host, JENKINS_URI_HISTORY_FEM101_EIFFEL012.port)
  request = Net::HTTP::Get.new("/#{URI_SUFFIX_HISTORY_FEM101_EIFFEL012}/api/json?tree=jobs[name,color]")
  if JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['name']
    request.basic_auth(JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['name'], JENKINS_AUTH_HISTORY_FEM101_EIFFEL012['password'])
  end
  response = http.request(request)
  if response.code == '200'
    ...
    $jenkins_jobs_to_be_tracked_fem101_eiffel012.each do |jenkins_job_name|
      search_for_job_in_full_job_list_json_fem101_eiffel012(build_info, jenkins_job_name, $most_recent_jenkins_status_map)
    end

    print "Updating job status history (file) ...\n"
    update_job_status_history_csv_file_fem101_eiffel012($most_recent_jenkins_status_map)

    print "loading job status history ...\n"
    $build_status_history_json = get_build_status_json_from_csv_file_fem101_eiffel012

    print "Sending event to build history widget ...\n"
    send_event('buildhistory_fem101_eiffel012', $build_status_history_json)
  end
end