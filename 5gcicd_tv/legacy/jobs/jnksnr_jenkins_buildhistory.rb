...
$jenkins_jobs_to_be_tracked_jnksnr = []
...
def get_jenkins_data_history_jnksnr(name_jenkins, key)
...
URI_SUFFIX_HISTORY_JNKSNR = get_jenkins_data_history_jnksnr('jnksnr-jenkins', 'uri_suffix')
JENKINS_URI_HISTORY_JNKSNR = URI.parse(get_jenkins_data_history_jnksnr('jnksnr-jenkins', 'uri'))
USER_HISTORY_JNKSNR = get_jenkins_data_history_jnksnr('jnksnr-jenkins', 'user')
PASS_HISTORY_JNKSNR = get_jenkins_data_history_jnksnr('jnksnr-jenkins', 'pass')

JENKINS_AUTH_HISTORY_JNKSNR = {
  'name' => USER_HISTORY_JNKSNR,
  'password' => PASS_HISTORY_JNKSNR
}

#function to get all job names from jenkins and save them in an array
def get_all_jobs_jnksnr
  http = Net::HTTP.new(JENKINS_URI_HISTORY_JNKSNR.host, JENKINS_URI_HISTORY_JNKSNR.port)
  request = Net::HTTP::Get.new("/#{URI_SUFFIX_HISTORY_JNKSNR}/api/json?tree=jobs[name]")
  if JENKINS_AUTH_HISTORY_JNKSNR['name']
    request.basic_auth(JENKINS_AUTH_HISTORY_JNKSNR['name'], JENKINS_AUTH_HISTORY_JNKSNR['password'])
  end
  all_jobs = []
...
def trim_filename_jnksnr(filename)
...
def get_build_status_json_from_csv_file_jnksnr
  ...
  CSV.foreach(JOB_STATUS_HISTORY_FILENAME) do |job_status_history_row|
    ...
    job_history_entry["job_name"] = trim_filename_jnksnr(job_name)
...
def update_job_status_history_csv_file_jnksnr(latest_jenkins_job_status_map)
  ...
  if (File.file?(JOB_STATUS_HISTORY_FILENAME) == false)
    #build array
    $jenkins_jobs_to_be_tracked_jnksnr.each do |job_name|
      job_entry = Array.new
...
def search_for_job_in_full_job_list_json_jnksnr(json_summary, job_name, job_status_hash_table)
...
SCHEDULER.every '60s', :first_in => 0 do
  ...
  $jenkins_jobs_to_be_tracked_jnksnr = get_all_jobs_jnksnr

  num_jobs = $jenkins_jobs_to_be_tracked_jnksnr.count()
  if num_jobs == 0
  ...
  http = Net::HTTP.new(JENKINS_URI_HISTORY_JNKSNR.host, JENKINS_URI_HISTORY_JNKSNR.port)
  request = Net::HTTP::Get.new("/#{URI_SUFFIX_HISTORY_JNKSNR}/api/json?tree=jobs[name,color]")
  if JENKINS_AUTH_HISTORY_JNKSNR['name']
    request.basic_auth(JENKINS_AUTH_HISTORY_JNKSNR['name'], JENKINS_AUTH_HISTORY_JNKSNR['password'])
  end
  response = http.request(request)
  if response.code == '200'
    ...
    $jenkins_jobs_to_be_tracked_jnksnr.each do |jenkins_job_name|
      search_for_job_in_full_job_list_json_jnksnr(build_info, jenkins_job_name, $most_recent_jenkins_status_map)
    end

    print "Updating job status history (file) ...\n"
    update_job_status_history_csv_file_jnksnr($most_recent_jenkins_status_map)

    print "loading job status history ...\n"
    $build_status_history_json = get_build_status_json_from_csv_file_jnksnr

    print "Sending event to build history widget ...\n"
    send_event('buildhistory_jnksnr', $build_status_history_json)
  end
end